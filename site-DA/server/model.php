...
function saveProfile($id, $name, $image, $minAge)
{
    try {
        ...
        if ($id > 0) {
                $sql = 'INSERT INTO Profil (id_profile, name_profile, image, min_age)
            ...
            $nextId = getNextId($cnx, 'Profil', 'id_profile');
            $sql = 'INSERT INTO Profil (id_profile, name_profile, image, min_age)
                VALUES (:id_profile, :name_profile, :image, :min_age)';
        $stmt = $cnx->prepare($sql);
...
function getAllProfiles()
{
    try {
        $cnx = getConnection();
        $sql = 'SELECT id_profile AS id, name_profile AS name, image, min_age FROM Profil ORDER BY name_profile';
        $stmt = $cnx->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll();
    } catch (Exception $e) {
...
function getFavorites($idProfile)
{
    try {
        ...
        $sql = 'SELECT m.id_movie AS id, m.name, m.image, m.description, m.director, m.year_movie, m.length, m.trailer, m.min_age, 
                        COALESCE(c.name, \'Sans categorie\') AS category
                FROM Favorite f
            JOIN Movie m ON f.id_movie = m.id_movie
            LEFT JOIN Category c ON m.id_category = c.id_category
...
function addRating($idProfile, $idMovie, $score)
{
    try {
        ...
        $sql = 'INSERT INTO Rating (id_rating, id_profile, id_movie, score, created_at)
            VALUES (:id_rating, :id_profile, :id_movie, :score, NOW())
            ON DUPLICATE KEY UPDATE
...
function addComment($idProfile, $idMovie, $commentText)
{
    try {
        ...
        $sql = 'INSERT INTO MovieComment (id_comment, id_profile, id_movie, comment_text, comment_status, created_at)
            VALUES (:id_comment, :id_profile, :id_movie, :comment_text, :comment_status, NOW())';
        $stmt = $cnx->prepare($sql);
...
function getMovieComments($idMovie)
{
    try {
        ...
        $sql = 'SELECT mc.id_comment AS id, mc.comment_text, mc.comment_status, mc.created_at, p.name_profile AS profile_name
            ...
            JOIN Profil p ON mc.id_profile = p.id_profile
...
function getTotalProfiles()
{
    try {
        $cnx = getConnection();
        $sql = 'SELECT COUNT(*) AS total_profiles FROM Profil';
        $stmt = $cnx->prepare($sql);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
    } catch (Exception $e) {
...
function getMostFavoritedMovie()
{
    try {
        ...
        $sql = 'SELECT m.id_movie AS id, m.name, COUNT(*) AS favorite_count
                FROM Favorite f
            JOIN Movie m ON f.id_movie = m.id_movie
            GROUP BY f.id_movie, m.id_movie, m.name
...
function getMostActiveProfile()
{
    try {
        ...
        $sql = 'SELECT p.id_profile AS id, p.name_profile AS name,
                ...
                FROM Profil p
            LEFT JOIN (SELECT id_profile, COUNT(*) AS fav_cnt FROM Favorite GROUP BY id_profile) fav ON p.id_profile = fav.id_profile
            LEFT JOIN (SELECT id_profile, COUNT(*) AS rate_cnt FROM Rating GROUP BY id_profile) r ON p.id_profile = r.id_profile
